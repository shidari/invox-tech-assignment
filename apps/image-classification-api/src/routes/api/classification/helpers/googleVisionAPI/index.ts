      ...
      async runAnnotateImageApi(imageUrl: string) {
        return safeTry(async function* () {
          ...
          if (!validatedResult.success) {
            const received = JSON.stringify(result, null, 2);
            const issues = JSON.stringify(validatedResult.issues, null, 2);
            return err(
              createGoogleVisionApiError(
                [
                  "Google Vision API response schema validation failed:",
                  "--- received ---",
                  received,
                  "--- issues ---",
                  issues,
